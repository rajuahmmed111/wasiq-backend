...
import { ICustomerContactFilters } from "./customerContact.interface";
import { pick } from "../../../shared/pick";
import { paginationFields } from "../../../constants/pagination";
import { CustomerContactValidation } from "./customerContact.validation";

// create customer contact
const createCustomerContact = catchAsync(async (req: Request, res: Response) => {
  const customerContactData = req.body;

  const result = await CustomerContactService.createCustomerContact(customerContactData);
...
const getAllCustomerContacts = catchAsync(async (req: Request, res: Response) => {
  const filters = pick(req.query, ["search", "minDate", "maxDate"]);
  const options = pick(req.query, paginationFields);

  const result = await CustomerContactService.getAllCustomerContacts(filters, options);
...
const getSingleCustomerContact = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
...
const updateCustomerContact = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
  const customerContactData = req.body;

  const result = await CustomerContactService.updateCustomerContact(id, customerContactData);
...
const deleteCustomerContact = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
